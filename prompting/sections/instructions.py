...
def section_instructions(cfg):
    ...
    return [
        ...
        "",
        "ONLY ADD ANIMATION. Do NOT edit the rest pose / bind pose of the rig. The rest pose is",
        "the model's neutral shape and must stay byte-for-byte unchanged; your script only layers",
        "keyframed POSE animation on top of it. Concretely, you MUST NOT:",
        "  - enter EDIT mode on the armature or change any edit_bone head/tail/roll/parent,",
        "  - call `bpy.ops.pose.armature_apply(...)` (apply pose as rest pose) or otherwise rebind,",
        "  - edit `armature.data.bones[...]` rest geometry, or move/rescale the armature object,",
        "  - leave any pose-bone transform set without a keyframe (an un-keyed pose silently",
        "    becomes the new resting look). Every pose-bone change must be a keyframe_insert.",
        "",
        ...
        "   `pose_bone.keyframe_insert(data_path=..., frame=...)`. Operate in POSE mode ONLY;",
        "   never enter EDIT mode and never apply the pose as the rest pose. The rest pose stays",
        "   exactly as imported — you are ADDING animation, not editing the base model.",
