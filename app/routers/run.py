...
def execute_bpy_code(code: str) -> dict:
    """Run a bpy script in a separate process; report what 3D file it produced.

    Shared by /api/run (code pasted by hand) and /api/chat (code generated by
    Gemini). Returns {ok, returncode, stdout, stderr, output_url}. Raises
    HTTPException(400) for an empty script or a timeout.

    SECURITY: this executes arbitrary Python — see the module docstring.
    """
